...
namespace HueScreenAmbience
{
	public class ZoneProcessor
	{
		...
		public void PostRead(PixelZone[] zones, int width, int height, int frame)
		{
			try
			{
				...
				//	smallImage.Write(writeStream, ImageMagick.MagickFormat.Png);
				//}

				using var image = _imageHandler.CreateImageFromZones(zones, width, height);
				...
				if (_config.Model.dumpPngs)
				{
					try
					{
						...
						using var writeStream = File.OpenWrite(path);
						image.Write(writeStream, ImageMagick.MagickFormat.Png);
